...
// 		chai.expect(event.name).to.be.an(FIELD_TYPE.STRING);
// 		chai.expect(event.inputValidator).to.be.an(FIELD_TYPE.OBJECT);
// 		chai.expect(event.outputValidator).to.be.an(FIELD_TYPE.OBJECT);
// 	});
// };
...
// const testValidationModels = (validationModels: ValidationSchema[]) => {
