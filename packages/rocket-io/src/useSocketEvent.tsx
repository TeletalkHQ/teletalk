...
export function useSocketEvent<T = any, EmitMessageCbReturnType = any>(
  event: string,
...
export function useSocketEvent<
  ...
  ListenMessageType = [T] extends [never] ? Parameter<ListenEvents[EventKey]> : T,
  EmitMessageArgs extends any[] = Parameters<EmitEvents[EventNames<EmitEvents>]>,
  //TODO: infer from last argument return type(cb) of EmitEvents[EventKey]
  // if last argument is a export function then infer return type
  // if last argument is not a export function then infer void
...
): UseSocketEventReturnType<ListenMessageType, EmitMessageArgs, EmitMessageCbReturnType>;
export function useSocketEvent<
  T = never,
  ListenEvents extends EventsMap = DefaultEventsMap,
  EmitEvents extends EventsMap = ListenEvents,
...
>(
  ...
  event: EventKey | (UseSocketEventOptions<ListenMessageType> & UseSocketOptions),
  options?: UseSocketEventOptions<ListenMessageType>
): UseSocketEventReturnType<ListenMessageType, EmitMessageArgs, EmitMessageCbReturnType> {
  let enabled = true;
  ...
  const connection = enabled ? getConnection((socket as SocketLike).namespaceKey) : null;
  ...
  const state = useRef<{
    ...
    socket: connection?.socket as SocketLike,
    status: connection?.state.status || 'disconnected',
    error: null,
    lastMessage: connection?.state.lastMessage[event as string] as ListenMessageType,
  });
